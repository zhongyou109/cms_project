<template>
    <div class="tmpl">
        ...
        <div class="pay-detail">
            <ul>
                <li v-for="(goods,index) in goodsList" :key="goods.nid"  class="p-list">
                    <mt-switch v-model="goods.isPicked"></mt-switch>
                    <img :src="goods.pic_url">
                    <div class="pay-calc">
                        <p v-text="goods.title"></p>
                        <div class="calc">
                            <span>￥{{goods.view_price}}</span>
...
export default {
    ...
    created(){
        ...
        //发起请求，将数据挂上
        this.$ajax.get('/src/static/json/book/book.json')
        .then(res=>{
            let products = res.data.mods.itemlist.data.auctions;
            ...
            products.forEach((ele,index)=>{
                if(prods[ele.nid]){ //数量
                    ...
                    // ele.isPicked = true;
                    //添加属性一定要this.$set
                    this.$set(ele,'num',prods[ele.nid]);
    ...
    computed:{
        payment(){
            ...
            this.goodsList.forEach((ele)=>{
                if(ele.isPicked){
                    //选中
                    sum += ele.num * parseFloat(ele.view_price);//总价
                    num += ele.num;//金额
                }
            })
            sum = Math.floor(sum * 100) / 100;//保留两位小数
            return {
                num,sum
            };
        }
